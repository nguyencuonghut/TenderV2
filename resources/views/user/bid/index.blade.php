              ...
              <div class="card-body">
                ...
                <table id="bids-table" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    @if($is_rating)
                    <td>
                      @if(count(array_keys($all_current_bid_prices, $bid->price)) > 1)
                        @if(0 == $my_key)
                          {{$my_key + 2}} | {{sizeof($all_current_bid_prices)}} bids
                        @else
                          {{$my_key + 1}} | {{sizeof($all_current_bid_prices)}} bids
                        @endif
                      @else
                        {{$my_key + 1}} | {{sizeof($all_current_bid_prices)}} bids
                      @endif
                    ...
                    <td>
                        @php
                            $bid_logs_cnt = App\Models\UserActivityLog::where('user_id', Auth::user()->id)->where('quantity_id', $bid->quantity_id)->where('activity_type', '<>', 'Xóa')->count();
                        @endphp
                            {{$bid_logs_cnt}}/5
                      </td>
                    @if('Đóng' != $tender->status
